...
:: Animación de inicio con opción de salto con Q
REM Primer frame
cls
echo (Presiona Q para saltar)
echo Running God Optimization Tool For Games
choice /c QN /n /t 1 /d N >nul
if errorlevel 2 (
    rem No se presionó Q, sigue
) else (
    cls
    goto :juego
)
...
:juego
set /p progname=Escribi el NOMBRE del ejecutable de tu juego (ej: notepad.exe) o M para poner la ruta manualmente (recomendado):

if /I "%progname%"=="M" (
    cls
    set /p ruta=Escribi la ruta completa del ejecutable:
    if "%ruta%"=="" (
        echo No se ha proporcionado una ruta. Saliendo...
        pause
        goto menu
    )
...
:: Buscar la ruta del ejecutable en todo el disco C:
set "ruta="
for /f "delims=" %%A in ('where /r C:\ "%progname%" 2^>nul') do set "ruta=%%A" & goto found

echo No se encontro el programa %progname% en tu PC.
pause
goto menu

:found
:: Extraer el nombre del ejecutable (ej: MiJuego.exe)
for %%A in ("%ruta%") do set nombre=%%~nxA
